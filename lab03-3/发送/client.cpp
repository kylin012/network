...
int window_size=1;
int ssth=64;
int state; //状态，0表示慢启动，1表示拥塞避免，2表示快速恢复
int last_ack; //上次接收到的ack序号
int dup_ack; //冗余ack的数量
int r_ack; //期望接收到的ack
...
// 输出阶段和窗口左边界和窗口大小和阈值大小
void printwin(){
    cout<<"state "<<state<<"\tl_window "<<left_edge<<"\twindow_size "<<window_size<<"\tssthresh "<<ssth<<endl;
}
...
void outtime(){
    while(1){
        ...
        if(!clock_on)
            continue;
        // 一直未能接收到正确的ACK信号，已超时
        // 超时的情况下一律进入慢启动状态，阈值变为窗口的一半，然后窗口大小变为1
        if((double)(clock()-time_st)>300){
            ssth = window_size/2;
            if(!ssth)ssth+=1;
            window_size=1;
            state = 0;
            ...
            for(int i = left_edge;i<=id;i++){
                sendto(client, send_ALL[i%MAX_SIZE], sizeof(group), 0, (sockaddr*)&saddr, sizeof(saddr));
                }
            time_st = clock();
        }
    }
...
void asend(){
    // 判断要发送的信息是否在窗口范围内
    while(1){
        if(id>=left_edge && id<left_edge+window_size)
            break;
    }
    id+=1;
    memset(send_ALL[id%MAX_SIZE], 0, sizeof(group));
	memcpy(send_ALL[id%MAX_SIZE], send_buf, sizeof(group)); //存入缓冲区
    sendto(client,send_buf,sizeof(group),0,(sockaddr*)&saddr, sizeof(saddr));
    group sg;
    ...
    while(1){
        ...
        if(ret>0 && rg.flags==2){
            ...
            if(rg.seq!=last_ack){
                ...
                if(state==0){
                    window_size*=2;
                    if (window_size >=ssth)state=1;
                    //if(window_size>MAX_SIZE)window_size=MAX_SIZE;
                }
                // 拥塞避免，窗口大小加1
                else if(state==1){
                    window_size+=1;
                ...
                // 快速恢复(new reno)，判断接收到的是RACK还是PACK
                else{
                    // RACK，设置窗口大小等于阈值，进入拥塞避免阶段
                    if(rg.seq>=r_ack){
                        window_size=ssth;
                        state=1;
                    }
            ...
            // 接收到冗余ack的情况，根据state分类讨论
                ...
                // 冗余ack达到三个，慢启动和拥塞避免都进入快速恢复阶段，并重传数据
                else if(dup_ack>=3){ 
                    ssth = window_size/2;
                    if(!ssth)ssth+=1;
                    // 进入快速恢复阶段时记录数据传输窗口的最高系列号
                    r_ack=left_edge+window_size-1;
                    window_size=ssth+3;
                    state=2;
                    for(int i = rg.seq;i<=id;i++){
                        sendto(client, send_ALL[i%MAX_SIZE], sizeof(group), 0, (sockaddr*)&saddr, sizeof(saddr));
